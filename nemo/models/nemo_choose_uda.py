...
from nemo.datasets.pascal_uda_part import MeshLoader, PartsLoader
...
class NeMo(BaseModel):
    def __init__(
        ...
        if cate in ['car', 'aeroplane', 'car_uda']:
            self.num_verts = 3072
        else:
            self.num_verts = 1024
        ...
        print('loaded parts: ', self.parts_loader.part_names)
    ...
    def evaluate_imagepart(self, sample, debug=False):
        ...
        seg_compare = seg[img_mask]
        bg_intersection = ((seg_compare == 0) & (anno_compare == 0)).sum()
        bg_union = ((seg_compare == 0) | (anno_compare == 0)).sum()
