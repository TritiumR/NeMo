...
class NeMo(BaseModel):
    ...
    def _build_inference(self):
        ...
        if self.inference_params.cameras.get('focal_length', None) is not None:
            self.inference_params.cameras['focal_length'] = self.inference_params.cameras[
                                                                'focal_length'] / self.down_sample_rate
        ...
        self.inter_module = MeshInterpolateModule(
            ...
            post_process=center_crop_fun(map_shape, (render_image_size,) * 2) if self.inference_params.get(
                'center_crop', False) else None,
            ...
            if dof == 6:
                self.samples_principal = torch.stack(
                    (torch.from_numpy(px_samples).view(-1, 1).expand(-1, py_samples.shape[0]) * map_shape[1],
                     torch.from_numpy(py_samples).view(1, -1).expand(px_samples.shape[0], -1) * map_shape[0]), ).view(2,
                                                                                                                      -1).T.to(
                    self.device)
